...
def add_node():
    with grpc.insecure_channel(os.getenv('MASTER_HOST') + ":" + os.getenv('MASTER_PORT')) as channel:
        stub = my_grpc.ProtocolStub(channel)
        response = stub.register_secondary_node(my_pb.Request(message=str(os.getenv('SECONDARY_HOST') + ":" + os.getenv('SECONDARY_PORT'))))
    log.info(f"Registered secondary node: {os.getenv('SECONDARY_HOST')}:{os.getenv('SECONDARY_PORT')}")
    return response.message

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    my_grpc.add_ProtocolServicer_to_server(protocol, server)
    server.add_insecure_port(os.getenv('SECONDARY_HOST') + ":" + os.getenv('SECONDARY_PORT'))
    server.start()
    log.info(f"Secondary node started: {os.getenv('SECONDARY_HOST')}:{os.getenv('SECONDARY_PORT')}")
